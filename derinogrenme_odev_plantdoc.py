...
if data_yaml_path.exists():
    ...
    if 'names' in data_yaml and isinstance(data_yaml['names'], list):
        ...
        for name in data_yaml['names']:
            turkish_name = translations.get(name, name) # Eğer çeviri yoksa orijinalini kullan
        ...
        print(f'⚠️ {data_yaml_path} içinde "names" anahtarı veya liste formatı bulunamadı. Güncelleme yapılamadı.')
else:
...
import torch
# Mevcut güncel sınıfları (Türkçe içeren) checkpoint'e tekrar kaydet
...
# =============================================================
#  PlantDoc Bitki Hastalığı Sınıflandırıcı
#  EfficientNet-B3 Transfer Learning | Google Colab Optimize
# =============================================================
#
#  Orijinal koddan yapılan iyileştirmeler:
#  ✓ AlexNet (scratch, 40 batch)  → EfficientNet-B3 / ResNet-50 (pretrained, tam eğitim)
#  ✓ Tek flip augmentation        → Zengin augmentation pipeline (crop, rotate, jitter, erase)
#  ✓ Sabit learning rate          → Warmup (5 ep) + Cosine Annealing
#  ✓ Yok                          → Mixed Precision (AMP) — GPU'da 2× hız
#  ✓ Yok                          → Gradient Clipping (max_norm=1.0)
#  ✓ Yok                          → WeightedRandomSampler (sınıf dengesizliği)
#  ✓ Yok                          → Sınıf ağırlıklı CrossEntropy + label smoothing
#  ✓ Yok                          → Backbone vs. Head için ayrı learning rate
#  ✓ Yok                          → Early stopping + best model checkpoint
#  ✓ Yok                          → Classification report, confusion matrix, eğitim grafikleri
#  ✓ Yok                          → Tekil görsel tahmin fonksiyonu (predict)
# =============================================================
...
if USE_AMP:
    ...
    print('  ⚠️  GPU yok! → Colab: Çalışma Zamanı > Donanım Hızlandırıcısı > T4 GPU')
print(f'{"="*60}\n')
...
# ─────────────────────────────────────────────────────────────
CFG = dict(
    zip_path     = 'plantdoc.zip',      # Colab'a yüklediğin zip adı
...
def _find_data_root(base: Path) -> Path:
    # En üst seviyede 'train' klasörünü ararız
    train_candidate = base / 'train'
    if train_candidate.is_dir():
        # Eğer 'train/images' ve 'train/labels' varsa, ana kök 'train'dir.
        if (train_candidate / 'images').is_dir() and (train_candidate / 'labels').is_dir():
            return train_candidate.parent # 'plantdoc_data' dönmeli

    # Daha genel bir arama: 'images' klasörünü içeren en üst klasörü bulmaya çalış
    img_dirs = [d for d in base.rglob('images') if d.is_dir()]
    if img_dirs:
        # images klasörünün bir üstü 'train' gibi düşünülebilir
        return img_dirs[0].parent.parent if img_dirs[0].parent.name == 'train' else img_dirs[0].parent
    return base # Bulamazsa ana kökü döndür

# Veri setinin ana kökünü bulalım (örn: ./plantdoc_data)
...
val_dir_imgs = main_data_root / 'valid' / 'images' # Genellikle 'valid' kullanılır
...
# Class mapping oluşturmak için önce tüm etiketleri oku
all_class_ids = set() # Changed to all_class_ids for clarity
for label_file in train_dir_labels.glob('*.txt'):
    with open(label_file, 'r') as f:
        # Her satırda 'class_id x_center y_center width height' formatında etiketler var
        for line in f:
            class_id = line.strip().split(' ')[0]
            all_class_ids.add(class_id)

# Sınıf ID'lerini (string olarak) alfabetik sıraya göre düzenle
...
# `data.yaml` dosyasından açıklayıcı sınıf isimlerini yükle
        ...
        with open(data_yaml_path, 'r') as f:
            ...
            if 'names' in data_yaml and isinstance(data_yaml['names'], list):
                for i, name in enumerate(data_yaml['names']):
                    id_to_descriptive_name[str(i)] = name # Map '0' to 'Apple_Apple_scab'
                print(f'✅ Sınıf isimleri {data_yaml_path} dosyasından yüklendi.\n')
            else:
                print(f'⚠️ {data_yaml_path} içinde "names" anahtarı veya liste formatı bulunamadı. Sayısal ID\'ler kullanılacak.\n')
    except Exception as e:
        print(f'❌ {data_yaml_path} okunurken hata oluştu: {e}. Sayısal ID\'ler kullanılacak.\n')
else:
    print(f'⚠️ {data_yaml_path} bulunamadı, sayısal ID\'ler kullanılacak.\n')

# Sınıf ID stringlerini (örn. '0', '1', '10') indekslere ve sonra açıklayıcı isimlere dönüştüren map'ler oluştur
# `class_to_idx`: '0' -> 0, '1' -> 1 (numerical string ID to integer index)
# `idx_to_class`: 0 -> 'Apple_Apple_scab' (integer index to descriptive name)

# `class_to_idx` ve `classes` (eğitim raporları için kullanılacak list) oluştur
class_to_idx = {class_id: i for i, class_id in enumerate(class_id_strings)}

# `classes` listesini açıklayıcı isimlerle doldur
# Eğer `id_to_descriptive_name` boşsa, varsayılan olarak sayısal string ID'leri kullan
classes = [id_to_descriptive_name.get(class_id, class_id) for class_id in class_id_strings]

# `idx_to_class` (integer index to descriptive name) oluştur
...
# ─────────────────────────────────────────────────────────────
# Custom PlantDoc Dataset Sınıfı
# ─────────────────────────────────────────────────────────────
class PlantDocDataset(Dataset):
    def __init__(self, img_dir, label_dir, class_to_idx, transform=None):
        self.img_dir = Path(img_dir)
        self.label_dir = Path(label_dir)
        self.class_to_idx = class_to_idx # This maps numerical string ID to integer index
        ...
        for img_path in sorted(list(self.img_dir.glob('*.jpg')) + list(self.img_dir.glob('*.jpeg')) + list(self.img_dir.glob('*.png'))):
            ...
            if label_path.exists():
                with open(label_path, 'r') as f:
                    ...
                    # Basit varsayım: Her dosyadaki ilk etiket, görselin ana sınıfını temsil eder.
                    # Eğer birden fazla nesne varsa, bu mantık iyileştirilmeli.
                    if labels_in_file:
                        class_id_str = labels_in_file[0].strip().split(' ')[0] # This is a numerical string ID (e.g., '0', '1', '10')
                        if class_id_str in self.class_to_idx:
                            class_idx = self.class_to_idx[class_id_str] # Convert to integer index
...
# Eğer ayrı bir validasyon seti yoksa, train setinden böl
if full_val_ds is None:
    ...
    test_ds  = val_ds # Ayrı test yoksa val seti test olarak kullanırız.

else:
    train_ds = full_train_ds
    val_ds   = full_val_ds
    test_ds  = full_test_ds if full_test_ds else full_val_ds # Eğer test yoksa validasyonu test olarak kullan


# ── WeightedRandomSampler: sınıf dengesizliğini gider ───────

# train_ds (Subset) için targets'ları çıkar
if isinstance(train_ds, Subset):
    # targets'ları orijinal dataset'ten alıp, subset indekslerine göre filtrele
...
# Test seti üzerinde toplu değerlendirme yap
print(f'🔍 Test seti değerlendiriliyor: {len(testloader.dataset)} görsel...')

# Modeli ve ağırlıkları tekrar kontrol et
ckpt = torch.load(CFG['save_path'], map_location=device)
model.load_state_dict(ckpt['model_state'])

# Değerlendirme fonksiyonunu çalıştır
_, test_acc, tpred, ttrue = run_epoch(model, testloader, optimizer, criterion, scaler, train=False)
